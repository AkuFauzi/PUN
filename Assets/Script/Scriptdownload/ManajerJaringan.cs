    ...
    public override void OnRoomListUpdate(List<RoomInfo> daftarRoom)
    {
       ...
       ClearRoomListView();
        ...
        foreach (RoomInfo room in cachedDaftarRoom.Values)
        {
            ...
            daftarEntriRoomGameobject.transform.Find("NamaRoomText").GetComponent<TextMeshProUGUI>().text = room.Name;
            daftarEntriRoomGameobject.transform.Find("MaksPlayerText").GetComponent<TextMeshProUGUI>().text = room.PlayerCount+ " / "+ room.MaxPlayers;
            daftarEntriRoomGameobject.transform.Find("ButtongabungRoom").GetComponent<Button>().onClick.AddListener(()=>OnJoinRoomButtonClicked(room.Name));
